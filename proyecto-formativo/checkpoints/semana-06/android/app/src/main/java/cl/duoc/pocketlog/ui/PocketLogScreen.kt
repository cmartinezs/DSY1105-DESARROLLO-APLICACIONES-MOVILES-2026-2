...
fun PocketLogScreen(
    ...
    Scaffold { padding ->
        Column(
            ...
            Text("Total ${state.total} · Pendientes ${state.pendientes} · Completados ${state.completados}")
            ...
            state.mensaje?.let { Text(it, style = MaterialTheme.typography.bodyMedium) }
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                FiltroEstado.entries.forEach { filtro ->
                    FilterChip(
                        ...
                        label = { Text(filtro.name.lowercase().replaceFirstChar(Char::uppercase)) }
                    )
                }
            }
            ...
            LazyColumn(
                ...
                items(state.registros, key = { it.id }) { registro ->
                    RegistroCard(registro = registro, onCompletar = { onCompletar(registro.id) })
                }
            }
        }
    }
...
@Composable
private fun RegistroCard(registro: Registro, onCompletar: () -> Unit) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            modifier = Modifier.padding(12.dp),
            verticalArrangement = Arrangement.spacedBy(6.dp)
        ) {
            Text("${registro.id}. ${registro.titulo}", style = MaterialTheme.typography.titleMedium)
